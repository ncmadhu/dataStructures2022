...
public class CountPalindromicSubstrings extends Problem {
    @Override
    public void run() {
        System.out.println("Running Count Palindromic Substring In String");
        String[] input = new String[]{"aabbbaa", "abcdef", "edfghaahgfde", "ababa"};
        this.execute(input);
    }
    ...
    private List<String> findAllPalindromeSubstringsBetterSolution(String input) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            count += this.getPalindromesBS(input, i, i, palindromes);
            // Even length Palindromes check
            count += this.getPalindromesBS(input, i, i+1, palindromes);
        }
        System.out.println("Total Palindromic Substrings: " + count);
        return palindromes;
    }

    private int getPalindromesBS(String input, int left, int right, List<String> palindromes) {
        int count = 0;
        while (left >= 0 && right < input.length()) {
            if (input.charAt(left) != input.charAt(right)) return count;
