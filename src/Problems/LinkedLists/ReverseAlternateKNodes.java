...
public class ReverseAlternateKNodes extends Problem {
    @Override
    public void run() {
        System.out.println("Running Reverse Alternate K Nodes In Linked List");
    ...
    private void execute(int[][] input) {
        for (int i = 0; i < input.length; i = i + 2) {
            ...
            list.displayLinkedList();
            list = LinkedListUtils.generateLinkedListFromArray(input[i]);
            list.head = this.reverseAltKNodes(list.head, input[i+1][0]);
            System.out.println("Output (Alternate K Nodes) : ");
            list.displayLinkedList();

        }
    }

    private LinkedListNode reverseAltKNodes(LinkedListNode head, int  k) {
        if (head == null || k <= 1) return head;
        LinkedListNode curr, prev, temp, currTail, prevTail;
        curr = head;
        prevTail = null;
        while (curr != null) {
            prev = null;
            ...
            if (prevTail == null) head = prev;
            else prevTail.next = prev;
            prevTail = currTail;
            prevTail.next = curr; // connect the prevTail to curr and move K nodes again for alternate K nodes
            n = k;
            while (n > 0 && curr != null) {
                prevTail = curr;
                curr = curr.next;
